...
public class MountainSplitterSkill extends WeaponInnateSkill implements ChargeableSkill {
    ...
    public MountainSplitterSkill(WeaponInnateSkill.Builder<?> builder) {
        super(builder);
                ...
                event -> {
                    if (skillContainer.getExecutor().isHoldingSkill(this)) {
                        ...
                        final PlayerInputState updated = current
                                .withForwardImpulse(current.forwardImpulse() * (1.0F - 0.9F * skillContainer.getExecutor().getSkillChargingTicks() / 45.0F));
    ...
    public void onStopHolding(SkillContainer container, SPSkillFeedback feedback) {
        container.getExecutor().getAnimator().getVariables()
                .put(EpicFightSynchedAnimationVariableKeys.CHARGING_TICKS.get(),
