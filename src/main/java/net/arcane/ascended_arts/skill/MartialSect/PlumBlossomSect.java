...
public class PlumBlossomSect extends MartialSectSkill{
    public PlumBlossomSect(SkillBuilder<? extends SkillBuilder> builder) {
        super(builder);
    }
                    ...
                    if (player.level().isClientSide) {
                        if (!player.isInWater() && !player.isCrouching() && !player.isFallFlying() && player.isSprinting()) {
                            ...
                            for (int i = 0; i < numParticles; i++) {
                                ...
                                if (basePos != null) {
                                    Particle particle1 = Minecraft.getInstance().particleEngine.createParticle(ParticleTypes.CHERRY_LEAVES, basePos.x, basePos.y, basePos.z, player.getDeltaMovement().x, 0.02F, player.getDeltaMovement().z);
                                    Particle particle2 = Minecraft.getInstance().particleEngine.createParticle(ParticleTypes.CHERRY_LEAVES, basePos.x, basePos.y + 0.26F, basePos.z, player.getDeltaMovement().x, 0.012F, player.getDeltaMovement().z);
                                    if (particle1 != null) {
